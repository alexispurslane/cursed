...
- **UTF-8 in wrap math** (part of #2). Wrap_rows, wrap_byte_offset, wrap_sub_position, mouse-click conversion, cursor rendering all assume 1 byte = 1 display column. CJK/tabs/combining marks mis-render and mis-click. Needs a `display_width(byte)` helper threaded through wrap math and paint.
...
- **Convenience functions for `global-set-key` and adding keys to major modes** [#5]. Additive helpers, not a `Keymap` class with parent pointers. Demotes the "keymaps aren't first-class" concern to a TODO for ergonomic wrappers.
...
- **Queries for C/Python/Go** (from scout report). Grammars registered, queries missing — only Lua and Rust actually highlight today. Pure completion gap, not architectural.
- **Mode registry for bash/json/toml/yaml** (scout). Grammars compiled in, no modes. Pure completion gap.
