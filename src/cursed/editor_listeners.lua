...
local function on_workspace_apply_edit(ed, params, rid, cid)
	...
	ed:apply_workspace_edit(ws_edit, function(r)
		...
		if n_touched > 0 then
			ed.status_message = ('applied "%s" (%d doc%s)'):format(label, n_touched, n_touched == 1 and "" or "s")
		elseif n_skipped > 0 then
			ed.status_message = ('applyEdit "%s": no open docs (%d skipped)'):format(label, n_skipped)
		else
...
local function on_render_squiggle_demo(ed)
	...
	local function ident(b)
		return b ~= nil and ((b >= 65 and b <= 90) or (b >= 97 and b <= 122) or (b >= 48 and b <= 57) or b == 95)
	end
	local pos = c.col + 1
	if pos > len then
		pos = len
...
local function on_render_diagnostic_hover(ed)
	...
	if entry ~= nil and view ~= nil and view.file_loaded then
		...
		if c ~= nil then
			...
			local best_start = -1
			for _, diag in ipairs(entry.items) do
				if diag.sl ~= nil and diag.el ~= nil and diag_contains(diag, view, buf, c.line, c.col, line_count) then
...
local function on_render_lsp_hover(ed)
	...
	local sig
	if view ~= nil and view.file_loaded and c ~= nil and view.buffer ~= nil and view.buffer.lsp_uri ~= nil then
		...
		if sig ~= nil and not hover_suppressed(ed) then
			...
			if cid ~= nil and uri ~= nil and lsp.is_ready(cid) then
				...
				ed._hover_task = ed:schedule_after(HOVER_DEBOUNCE_US, function()
					...
					local hover_id = lsp.request_hover(captured.cid, captured.uri, { line = p.line, character = char })
