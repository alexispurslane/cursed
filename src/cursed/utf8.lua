...
---@param s string
---@param tab_width? integer tab stop width (0 = no tab expansion, default nil)
---@return integer[] byte_starts
---@return integer[] widths
---@return integer[] prefix
function utf8.parse_line(s, tab_width)
	...
	while i <= n do
		...
		-- Override tab width when tab_width is set
		if cp == 9 and tab_width and tab_width > 0 then
...
--- Word-wrap computation: given grapheme arrays from `parse_line`, compute
--- sub-row indices and display columns for each grapheme. Pure function
--- extracted from `View:_wrap_graph` — no view/cache dependencies.
---
--- Returns per-grapheme arrays parallel to `byte_starts`:
---   sub_rows[i] = 0-based sub-row index of grapheme i's start
---   sub_cols[i] = 0-based display column within that sub-row
---   total_rows = total number of sub-rows (>= 1)
---   sub_first[r] = first grapheme index in 0-based sub-row r
---   sub_last[r]  = last grapheme index in 0-based sub-row r
--- Uses word-wrap: when a grapheme would overflow wrap_width, the
...
--- If no word boundary exists, falls back to grapheme-level break.
--- Over-wide graphemes (gw > wrap_width) occupy their own row.
---@param byte_starts integer[]
---@param widths integer[]
---@param text string line text (for space/tab detection via byte lookup)
...
function utf8.compute_wrap(byte_starts, widths, text, wrap_width)
	local ng = #widths
	local sub_rows, sub_cols = {}, {}
	if ng == 0 then
		return sub_rows, sub_cols, 1, nil, nil
	end
	-- No wrapping: everything on sub_row 0; sub_col = cumulative column.
	...
	for i = 1, ng do
		...
		if over_wide then
			...
			if text and byte_starts then
				local b = text:byte(byte_starts[i])
	...
	return sub_rows, sub_cols, total, sub_first, sub_last
end

--- Wrap a plain string at word boundaries to a target width.
--- Pure function, no view dependencies. Uses `parse_line` + `compute_wrap`
--- internally, then builds a list of wrapped lines (strings without trailing
--- newlines). Trailing whitespace on each line is stripped.
---@param text string input text (may contain newlines)
---@param width integer wrap width in display columns
---@return string[] lines
function utf8.wrap_string(text, width)
	if #text == 0 then
		return { "" }
	end
	local byte_starts, widths = utf8.parse_line(text)
	widths = widths or {}
	local total, sub_first, sub_last = select(3, utf8.compute_wrap(byte_starts, widths, text, width))

	if total <= 1 then
		-- No wrapping needed (or width <= 0)
		return { (text:gsub("%s+$", "")) }
	end

	local lines = {}
	for r = 0, total - 1 do
		local first = sub_first and sub_first[r]
		local last = sub_last and sub_last[r]
		if first and byte_starts[first] then
			local start_byte = byte_starts[first]
			local end_byte
			if last and last < #byte_starts then
				end_byte = byte_starts[last + 1] - 1
			else
				end_byte = #text
			end
			---@cast end_byte integer
			local line = text:sub(start_byte, end_byte)
			-- Strip trailing whitespace (the word-wrap algorithm keeps
			-- a space as the trailing character on each row).
			line = line:gsub("%s+$", "")
			lines[#lines + 1] = line
		else
			lines[#lines + 1] = ""
