...
commands.move_beginning_of_visual_line = function(view, _editor)
	view:move_beginning_of_visual_line()
end

commands.move_end_of_visual_line = function(view, _editor)
...
commands.move_beginning_of_visual_line_select = function(view, editor)
	editor._extend = true
	view:_begin_shift_select()
	view:move_beginning_of_visual_line()
end

commands.move_end_of_visual_line_select = function(view, editor)
	editor._extend = true
	view:_begin_shift_select()
	view:move_end_of_visual_line()
end
...
----------------------------------------------------------------------------------------------------
-- Sentence spans (motion / copy / mark / kill)
--
...
-- between-units contract + doc-end guards); kill / copy / drag
...
----------------------------------------------------------------------------------------------------
--- Toggle the `wrap` display option on the current view.
--- When off, soft wrap still wraps at window width but skips margin narrowing.
commands.toggle_wrap = function(_view, editor)
...
----------------------------------------------------------------------------------------------------
-- Drag: neovim-style selection dragging
--
-- Alt-left / Alt-right moves the active selection one step in the
-- given direction, deleting its text from the old position and
-- inserting it at the new one. When the selection was created by a
-- named textobject (mark_word, forward_word_select, …), the step
-- jumps over one adjacent unit of that SAME textobject (the inter-unit
-- gap is preserved). When the selection has no known textobject (manual
-- mark, shift-select from arrow keys, expand-region, …), the step
-- nudges one character instead. Repeats with a universal arg.
--
-- After a drag the selection is re-established at the INSERTED text's
-- new span (mark at start, point at end), and the textobject attribution
-- is preserved so successive drags keep bounding over units. Dragging
-- off either document edge reports a status message and leaves the
-- selection in place.
--
-- Primary-cursor only (mirrors the single-cursor scope this
-- feature replaces): the coordinate math — one deletion + one
-- insertion, plus its own cursor translator — stays in a single
-- well-understood path).
----------------------------------------------------------------------------------------------------

--- Apply one drag step of the primary cursor's selection in `dir` (+1
