...
---@field wrap boolean default true; when false, soft wrap skips margin narrowing (wraps at window width only)
...
---@field _screen_rows_cache table[]|nil cache: _screen_rows_cache[li+1] = ScreenRow[] — per-line pre-built screen rows. Keyed on _graph_gen + wrap_width.
...
---@field width integer total display width

---@class PropertiesSpan
---@field col_s integer 0-based start column
---@field row_s integer 0-based start line
---@field col_e integer 0-based end column (exclusive)
...
---@class LayerSpan
---@field type '"layer"'
---@field draw fun(term: table, sx: integer, sy: integer)|nil screen-space overlay draw function
---@field fn fun(...)|nil alternative draw callback
---@field col_s? nil no buffer coordinate
---@field row_s? nil
---@field col_e? nil
---@field row_e? nil
...
---@class AnchorSpan
---@field type '"anchor"'
---@field col_s integer buffer position (point, not range)
---@field row_s integer
---@field col_e integer|nil same as col_s (range is same as point)
---@field row_e integer|nil same as row_s
---@field draw fun(term: table, sx: integer, sy: integer) resolved screen-coords draw function
...
function View:set_major_modes(modes)
    ...
    if last_major then
        ...
        self.margin = last_major.margin ~= nil and last_major.margin
            or (self.editor and self.editor.margin)
...
---@param opts { no_gutter: boolean?, no_line_numbers: boolean?, wrap: boolean?, no_wrap: boolean?, whole_line_cursor: boolean?, no_completion: boolean? }
function View:change_display_opts(opts)
    ...
    -- `wrap` is the canonical toggle; accept `no_wrap` for backward compat.
    local new_wrap
    if opts.wrap ~= nil then
        new_wrap = opts.wrap
    elseif opts.no_wrap ~= nil then
        new_wrap = not opts.no_wrap
    end
    if new_wrap ~= nil and new_wrap ~= self.wrap then
        self.wrap = new_wrap
...
function View:text_geometry(w)
    ...
    if self.no_gutter then
        local margin = self.wrap and self.margin
...
function View:wrap_rows(li)
    ...
    -- Use _screen_row_counts when already populated by a prior
    -- _screen_rows call. First access builds ScreenRows, which also
    -- populates _wrap_graph_cache for the unified O(log N) cursor index.
    if self._screen_row_counts and self._screen_row_counts[li + 1] then
        return self._screen_row_counts[li + 1]
    end
    ...
    for li in pairs(affected) do
        if structural[li] then
            ...
            if self._graph_cache[li + 1] then
                ...
                if self._screen_rows_cache then
                    self._screen_rows_cache[li + 1] = nil
                end
                if self._screen_row_counts then
                    self._screen_row_counts[li + 1] = nil
                end
            end
        else
...
function View:_graph_update_line(li)
    ...
    local new_bs, new_w, _ = utf8.parse_line(inserted_text, self.tab_width)
    ...
    -- Invalidate ScreenRows cache for this line (it depends on
    -- byte_starts/widths).
    if self._screen_rows_cache then
        self._screen_rows_cache[li + 1] = nil
    end
...
local function ensure_graph_gen(view)
    ...
    local tw = view.tab_width
    if view._graph_gen ~= gen or view._graph_tab_width ~= tw then
        view._graph_cache = nil
        view._graph_line_text = nil
        view._wrap_graph_cache = nil
        view._wrap_graph_gen = nil
        view._graph_gen = gen
        view._graph_tab_width = tw
    end
end
...
function View:_graph(li)
    ...
    local bs, w, p = utf8.parse_line(text, self.tab_width)
...
function View:_wrap_graph(li)
    ...
    local sub_rows, sub_cols, total_rows, sub_first, sub_last =
        utf8.compute_wrap(bs, widths, text, ww)
    entry = {
        sub_rows = sub_rows,
        sub_cols = sub_cols,
...
----------------------------------------------------------------------------------------------------
-- Token stream, ScreenRows, and token-aware wrapping
----------------------------------------------------------------------------------------------------

--- Finalize the current screen row and append it to `rows`.
--- Makes a shallow copy of `row_segs` so the caller can safely clear/reuse the table.
---@param rows ScreenRow[]
---@param row_segs RowSegment[] (shallow-copied; caller may reuse)
---@param buf_start integer 0-based byte offset where this row starts
---@param li integer logical line index
local function _flush_row(rows, row_segs, buf_start, li)
    if #row_segs == 0 then
        return
    end
    local buf_end = row_segs[#row_segs].buf_end

    -- Compute width as the rightmost extent of any segment on this row.
    local width = 0
    ...
    -- Shallow-copy the segments array so the caller can reuse row_segs.
    local segs_copy = {}
    for i = 1, #row_segs do
        segs_copy[i] = row_segs[i]
    end
...
--- Word-wrap a TEXT token into the accumulating row_segs.
--- Walks grapheme widths, tracking last space/tab offset for word-boundary
--- breaks. When overflow is detected, backtracks to the last space (if any)
--- or breaks at grapheme level.
---@param tok LineToken (type="text")
---@param ww integer wrap width
---@param ws table mutable WrapState {row_buf, col, last_space_byte, last_space_gi}
---@param rows ScreenRow[] (appended to when a row is flushed)
---@param row_segs RowSegment[] (current row accumulation)
---@param li integer logical line index
local function _wrap_text_token(tok, ww, ws, rows, row_segs, li)
    ...
    local col = ws.col
    local seg_gi = 1 -- 1-based grapheme index in tok where the current segment starts
    local seg_col = ws.col -- display column of seg_gi within the row

    local i = 1
    while i <= ng do
        local gw = widths[i]

        -- Check for space/tab at this grapheme (word-wrap tracking).
        local b = tok.text:byte(byte_offsets[i] - tok.buf_start + 1)
        if b == 0x20 or b == 0x09 then
            ws.last_space_byte = byte_offsets[i]
            ws.last_space_gi = i
        end

        if col + gw > ww and col > 0 then
            -- Overflow: determine break point.
            local break_gi -- 1-based grapheme index to break BEFORE

            if
                ws.last_space_byte
                and ws.last_space_gi
                and ws.last_space_gi >= seg_gi
                and ws.last_space_byte > ws.row_buf
            then
                -- Word-wrap: break after the last space (space stays on current row).
                break_gi = ws.last_space_gi + 1
            else
                -- Grapheme-level break: break before the overflowing grapheme.
                break_gi = i
            end

            -- Emit TEXT segment for [seg_gi, break_gi-1] if any.
            if break_gi > seg_gi then
                local t_text = tok.text:sub(
                    byte_offsets[seg_gi] - tok.buf_start + 1,
                    byte_offsets[break_gi] - tok.buf_start
                )
                local t_widths = {}
                for j = seg_gi, break_gi - 1 do
                    t_widths[#t_widths + 1] = widths[j]
                end
                local seg_bo = {}
                for j = seg_gi, break_gi - 1 do
                    seg_bo[#seg_bo + 1] = byte_offsets[j] - byte_offsets[seg_gi]
                end
                table.insert(row_segs, {
                    type = "text",
                    buf_start = byte_offsets[seg_gi],
                    buf_end = byte_offsets[break_gi],
                    text = t_text,
                    col = seg_col,
                    widths = t_widths,
                    byte_offsets = seg_bo,
            ...
            -- Flush the row and start a new one.
            local cur_row_buf = ws.row_buf
            ws.row_buf = byte_offsets[break_gi]
            _flush_row(rows, row_segs, cur_row_buf, li)
            -- Clear row_segs so the caller knows it was flushed.
            ...
            -- Re-process this grapheme if we broke before it;
            -- continue from the next grapheme if we broke after the space.
            if break_gi <= ng then
                i = break_gi
            else
                break
    ...
    -- Emit remaining graphemes as a segment (unflushed, stays in row_segs).
    if seg_gi <= ng then
        local t_text = tok.text:sub(byte_offsets[seg_gi] - tok.buf_start + 1)
        local t_widths = {}
        for j = seg_gi, ng do
            t_widths[#t_widths + 1] = widths[j]
        end
        local seg_bo2 = {}
        for j = seg_gi, ng do
            seg_bo2[#seg_bo2 + 1] = byte_offsets[j] - byte_offsets[seg_gi]
        end
        table.insert(row_segs, {
            type = "text",
            buf_start = byte_offsets[seg_gi],
...
---@return ScreenRow[] (always length 1)
function View:_build_unwrapped_rows(li, tokens)
    local row_segs = {}
    local col = 0
    local buf_start = (tokens[1] and tokens[1].buf_start) or 0
...
function View:_wrap_line(li)
    ...
    local ww = self.wrap_width or 0
    if ww <= 0 then
        return self:_build_unwrapped_rows(li, tokens)
    end

    -- WrapState: tracks the current row being built.
    local ws = {
        row_buf = 0, -- 0-based byte offset of the first content on the current row
        col = 0, -- current display column within the current row
        last_space_byte = nil, -- 0-based byte offset of last space/tab on this row
        last_space_gi = nil, -- 1-based grapheme index of last space/tab on this row
    }
...
---@param li integer 0-based line index
---@return ScreenRow[]
function View:_screen_rows(li)
    ensure_graph_gen(self)
    local ww = self.wrap_width or 0
    local cache = self._screen_rows_cache
    if
        cache == nil
        or self._screen_rows_gen ~= self._graph_gen
        or self._screen_rows_width ~= ww
    then
        cache = {}
        self._screen_rows_cache = cache
        self._screen_row_counts = {}
        self._screen_rows_gen = self._graph_gen
        self._screen_rows_width = ww
    end
    ...
    local rows = self:_wrap_line(li)
    cache[li + 1] = rows
    self._screen_row_counts[li + 1] = #rows
    ...
    if ww > 0 then
        ...
        if ng > 0 then
            local sub_rows, sub_cols = {}, {}
            local sub_first, sub_last = {}, {}

            -- Initialize: graphemes default to sub_row 0, col 0.
            -- TEXT segments overwrite with actual positions.
            for i = 1, ng do
                sub_rows[i] = 0
                sub_cols[i] = 0
            end

            local function grapheme_at(byte_offset)
                local target = byte_offset + 1
                local lo, hi, gi = 1, ng, 1
                while lo <= hi do
            ...
            local wg_cache = self._wrap_graph_cache
            if wg_cache == nil then
                wg_cache = {}
                self._wrap_graph_cache = wg_cache
            end
            wg_cache[li + 1] = {
                sub_rows = sub_rows,
                sub_cols = sub_cols,
...
function View:wrap_byte_offset(li, sub_row, sub_col)
    ...
    -- Unified path: _screen_rows populates _wrap_graph_cache for all lines.
...
function View:wrap_sub_position(li, byte_offset)
    ...
    if byte_offset >= ll then
        return sub_rows[ng], sub_cols[ng] + widths[ng]
    end
    local target = byte_offset + 1
...
--- When soft wrap is inactive, delegates to move_line_start.
function View:move_beginning_of_visual_line()
    return self:each_cursor(function(c)
        if not self.wrap_width or self.wrap_width <= 0 then
            c.col = 0
            c.goal_col = 0
        ...
        local li = c.line
        local sub_row = self:wrap_sub_position(li, c.col)
        local bs, _, _, _ = self:_graph(li)
        local _, _, _, sub_first = self:_wrap_graph(li)
        local first_gi = sub_first and sub_first[sub_row]
        if first_gi then
            c.col = bs[first_gi] - 1 -- 1-based byte → 0-based byte
        else
            c.col = 0
        end
        c.goal_col = 0
...
--- Move the primary cursor to the end of the current visual sub-row.
--- When soft wrap is inactive, delegates to move_line_end.
function View:move_end_of_visual_line()
    return self:each_cursor(function(c)
        if not self.wrap_width or self.wrap_width <= 0 then
            local content_len = self.buffer:line_len(c.line) - 1
            c.col = content_len
        ...
        local sub_row = self:wrap_sub_position(li, c.col)
        local bs, _, _, ll = self:_graph(li)
        local _, _, _, _, sub_last = self:_wrap_graph(li)
        ...
        if last_gi then
            if last_gi < #bs then
                ...
                -- subtract 1 for the preceding byte (1-based), subtract
                -- another 1 to convert from 1-based to 0-based.
                c.col = bs[last_gi + 1] - 2
            else
                c.col = ll -- past the last visible byte (= the newline)
            end
        else
            c.col = buf:line_len(li) - 1
        end
        c.goal_col = self:byte_to_col(li, c.col)
        c.visual_col = nil
        c.yank_line = nil
        c.yank_col = nil
        return true
...
--- Implementation: the document is treated as alternating UNITS
--- (text) and GAPS (boundary match + trailing whitespace). gap_end
--- skips ALL whitespace past a match (newlines + blank lines) so
--- paragraph-spanning cases need no special handling; unit_end
--- (_boundary_prefix_end) keeps the terminator with the preceding
--- unit. on_boundary is a single byte-range comparison.
...
function View:_pattern_range(pat, line, col, dir)
    local buf = self.buffer
    -- The document is an alternating sequence of UNITS (text) and GAPS
    -- (boundary matches + trailing whitespace). A unit is the text
    -- between two boundary matches. This function finds the unit
    -- containing/adjacent to point and returns it as a half-open
    -- [start, end) range plus the trailing-gap (boundary_len) so forward
    -- motion can land at the next unit's start.
    --
    -- Two primitives keep the model uniform across words, sentences,
    -- subsentences, and their newline/blank-line/paren edge cases:
    --
    --   gap_end(m)  — advance PAST boundary match `m` and ALL trailing
    --                whitespace (spaces, tabs, newlines, blank lines)
    --                to the first char of the NEXT unit. This is the
    --                sole place newline/blank-line handling lives, so
    --                every caller gets it for free — no per-call
    --                skip_blank flag, no para_start fallback, no
    --                inverted-span re-search.
    --
    --   unit_end(m) — (_boundary_prefix_end) the preceding unit ends at
    --                the terminator portion of the match: the non-ws
    --                prefix belongs to the unit (so `. ` keeps its `.`),
    --                the trailing ws is the gap. A pure-separator match
    --                (ALL non-ws, e.g. `(` for words) has no terminator,
    --                so the unit ends at the match START and the whole
    --                match is the gap. This is the ONE design decision
    --                (terminator-in-unit); expressed in one place.
    local function gap_end(m)
        local li = m.end_line
        local co = m.end_offset
        while true do
            local cl = self:content_len(li)
            ...
            while co < tlen do
                local b = text:byte(co + 1)
                if b ~= 32 and b ~= 9 then
                    -- Non-ws on this line: unit starts here.
                    return li, co
                end
                co = co + 1
            end
            -- Exhausted this line. Advance to the next; blank lines are
            -- skipped naturally (tlen==0 → loop body does nothing,
            -- co stays 0, fall through to next line).
            if li + 1 >= self:line_count() then
                return li, self:content_len(li)
            end
            li = li + 1
            co = 0
        end
    end
    -- Fallback unit start when there's no preceding boundary before
    -- point (the first unit of a paragraph/file): the first non-ws char
    -- of the paragraph run containing `line`. gap_end can't produce this
    -- because there's no match to anchor on.
    local function para_start(linecol)
        local top, _, _, _ = self:paragraph_range(linecol)
        top = top or linecol
        local text = buf:line_text(top)
        local tlen = #text
        if tlen > 0 and text:byte(tlen) == 10 then
            tlen = tlen - 1
        end
    ...
    -- Point is ON a gap if an inclusive backward search from pt lands
    -- within a match's byte span [offset, end_offset). byte-offset makes
    -- the multi-line match case a single comparison.
    local binfo = buf:search_backward(pat, pt, false)()
    local on_boundary = false
    if binfo then
        local pb = self:line_col_byte_offset(line, col)
        local mb_s = self:line_col_byte_offset(binfo.line, binfo.offset)
        local mb_e = self:line_col_byte_offset(binfo.end_line, binfo.end_offset)
        on_boundary = pb >= mb_s and pb < mb_e
    end
    if not on_boundary then
        -- Inside a unit: [prev-gap-end, cur-unit-end).
        local sl, sc = para_start(line)
        if prev_m then
            sl, sc = gap_end(prev_m)
        end
        if not cur_m then
            local e = self:eof_pt()
    ...
    -- On a gap. dir selects the adjacent unit.
    if dir == 0 then
        return nil, nil, nil, nil, nil
    end
    if dir == nil or dir > 0 then
        -- Forward: the unit AFTER this gap. Start = gap_end(binfo).
        -- End = the NEXT boundary strictly past the gap — NOT cur_m
        -- (which boundaries_pat found from pt and may BE binfo itself
        -- when point sits on the match). Re-search from the unit start.
        local sl, sc = gap_end(binfo)
        local _, next_m = self:boundaries_pat(pat, { line = sl, offset = sc })
        if not next_m then
            local e = self:eof_pt()
            return sl, sc, e.line, e.offset, 0
        end
        local el, ec, tws = self:_boundary_prefix_end(next_m)
        return sl, sc, el, ec, tws
    end
...
---
--- Pure-separator rule: when the ENTIRE match is non-whitespace (no
--- trailing ws), the matched chars are a pure separator (e.g. word's
--- `-` / `,`), NOT a terminator that belongs to the preceding unit.
--- In that case the unit ends at m.offset (before the separator) and
--- boundary_len spans the whole match — so the separator is treated
--- like whitespace: it's the gap, not part of either unit. This keeps
--- mark_word on "beta" in "beta,gamma" from selecting "beta,".
...
function View:_boundary_prefix_end(m)
    ...
    -- Entirely non-whitespace match => pure separator, not a unit terminator.
    if nws == #mtext then
        return m.line, m.offset, #mtext
    end
        ...
        for _ = 1, remaining do
            ...
            if forward then
                ...
                if sl == c.line and sc == c.col then
                    ...
                    local pline = c.line
                    if pcol < 0 then
                        -- Walk back to the previous non-blank line so we land
                        -- in real content (blank lines are gaps pattern
                        -- textobjects don't see; a single-line step-back
                        -- would stranding us on the blank line, querying
                        -- the same prev boundary and looping).
                        pline = c.line - 1
                        while pline > 0 and self:is_blank_line(pline) do
                            pline = pline - 1
                        end
                        if pline < 0 or (pline == 0 and self:is_blank_line(0)) then
                            return nil, "start of document"
                        end
