...
function View:_rebuild_highlighter()
	...
	if self._hl_lang == lang and self._hl_query == query and self._hl_injection_query == inj_query then
...
function View:_hl_generation()
	local cs = require("cursed.colorscheme")
	local scheme_gen = cs.generation or 0
	return tonumber(self.buffer._ptr.undo.count) + tonumber(self.buffer._ptr.redo.count) + scheme_gen * 1000000
end
...
function View:_hl_wait_response()
	...
	while true do
		...
		if cur == nil or cur.gen ~= target_gen then
			...
			log.info("view", "hl_wait ok", { gen = target_gen, elapsed_us = elapsed_us, iters = iters })
			return true
		end
		pffi.C.gettimeofday(tv, nil)
...
function View:_hl_install_spans(gen, bucket_start, bucket_end, count, msg_ptr, spans_ptr, name_count, names_ptr)
	if not self._hl_enabled then
		return false
	end
	...
	elseif n_written > 0 then
		...
		for i = 0, count - 1 do
			local s = spans_ptr[i]
			local cap_name = names[tonumber(s.cap_index) + 1]
			local fg = (scheme ~= nil and cap_name ~= nil) and scheme:resolve_capture(cap_name) or nil
			local eb = tonumber(s.end_byte)
			local sb = tonumber(s.start_byte)
			if fg ~= nil and eb > sb then
...
function View:wrap_sub_position(li, byte_offset)
	...
	-- Position past the last grapheme (byte_offset >= content_len):
	-- return the last grapheme's END position on its sub-row. Without
	-- this special case, the binary search below snaps to the last
	-- grapheme's START col, off-by-one — affects squiggle / overlay
	-- ranges whose e_col lands exactly at content_len (e.g. when a
	-- misspelled word ends a line). Matches `byte_to_col`'s no-wrap
	-- early return (prefix[n] + widths[n]).
...
local function is_word_byte(b)
	...
	return (b >= 0x41 and b <= 0x5A) or (b >= 0x61 and b <= 0x7A) or (b >= 0x30 and b <= 0x39) or b == 0x5F
end
		...
		while col >= 0 do
			...
			if op_str then
				...
				if mf_cl then
					...
					local before_close = (mf_cl < pli) or (mf_cl == pli and (mf_cc + (mf_clen or 0)) <= pcol)
...
function View:sexp_range(line, col, open_of, close_of, dir)
	...
	local ol, oc, cl, cc, olen, clen = self:enclosing_pair({ line = line, offset = col }, open_of, close_of)
