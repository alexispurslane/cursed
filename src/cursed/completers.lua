...
----------------------------------------------------------------------------------------------------
-- fzy-based filtering helper (replaces ad-hoc substring/prefix filtering)
----------------------------------------------------------------------------------------------------

--- Score, filter, sort, and cap items by fzy match against a query.
--- Higher-scoring items come first. Empty query returns first N items
--- unchanged (all pass). Non-matching items (nil score) are removed.
---@generic T
---@param items T[]
---@param text_fn fun(item: T): string extracts the searchable text from an item
---@param query string the user's query text
...
----------------------------------------------------------------------------------------------------
-- Prefix-matching filter (for in-buffer completion menu sources)
--
-- Preserves original item order. Case-insensitive prefix matching,
-- which is the expected behaviour for dabbrev-style word completion
-- and LSP client-side filtering — fuzzy matching for these feels
-- unintuitive (the user types "re" and expects "read", not "are").
-- Other completer sources (M-x, find-file, buffers, symbols) use
-- fzy_filter above since navigation/palette tasks benefit from fzy.
----------------------------------------------------------------------------------------------------

--- Filter items to those whose searchable text starts with `prefix`
--- (case-insensitive). Original order is preserved; no cap.
---@generic T
---@param items T[]
---@param text_fn fun(item: T): string
---@param prefix string
---@return T[]
