...
---   Mode (or MajorMode/MinorMode alias) — constructor for custom modes
...
---     modes = {
---       mything = Mode.new{ name = "mything", tab_width = 2, ... },
---     },
---
---     -- Ordered { pattern, mode } pairs. `mode` MUST be a `modes.<name>`
---     -- Mode object (built-in or user) — bare name strings are
...
---@class Config
---@field modes table<string, Mode> name → Mode (built-ins + user)
---@field file_patterns { [1]: string, [2]: Mode }[] normalized {pattern, Mode}
...
-- in src/cursed/modes/<lang>.lua return). At load time we run Mode.new
...
---@param modes table<string, Mode> table to merge into
...
--- Resolve a file_patterns mode reference to a Mode object.
--- The ONLY accepted form is a Mode object (a constructed mode —
...
function Config.load(editor)
	...
	local modes = {} ---@type table<string, Mode>
	for name, mode in pairs(builtin.modes) do
		modes[name] = mode
	end

	local file_patterns = {} ---@type { [1]: string, [2]: Mode }[]
	...
	if result and result.keybindings ~= nil and type(result.keybindings) == "table" then
		for chord, action in pairs(result.keybindings) do
			if type(chord) == "string" and (type(action) == "string" or type(action) == "function") then
